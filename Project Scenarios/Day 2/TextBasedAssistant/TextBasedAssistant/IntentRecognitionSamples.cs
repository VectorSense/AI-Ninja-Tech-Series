            ...
            namespace Speech
            {
                class IntentRecognitionSamples
                {
                    ...
                    public static async Task TextBasedAssistant()
                    {
                       ...
                       // var config = SpeechConfig.FromSubscription("b3f8ff6e7d2848169cb1890fafe1cce4", "westus");
                        var config = SpeechConfig.FromSubscription("", "westus2");
                        using (var recognizer = new IntentRecognizer(config))
                        {
                            // Creates a Language Understanding model using the app id, and adds specific intents from your model
                            var model = LanguageUnderstandingModel.FromAppId("fae5b3b6-d981-4c13-8192-e89cd4ef7224");
